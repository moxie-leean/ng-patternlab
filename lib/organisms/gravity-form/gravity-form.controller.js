...
function lnOGravityFormController($log, $scope, $element, $attrs, $sce, lnOGravityFormService, lnOGravityValidationParser) {
  ...
  function gatherFormData() {
    ...
    for (var i = 0; i < inputs.length; i++) {
      ...
      if (!!input.name && input.name !== '') {
        ...
        // If it's a multiselect type, then we need to catch an array of values
        if (input.multiple === true) {
          var _selected = angular.element(input).val();
          formData.input_values[input.name] = _selected;
          continue;
        }
  ...
  function clearAllValidationBlocks() {
    if (angular.isDefined(validationBlocks)) {
      angular.element(validationBlocks).detach('p');
    }
  }
