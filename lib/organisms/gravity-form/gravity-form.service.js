(function () {
                        ...
                        function (response) {
                            if (!!response.data.status && response.data.status === 200) {
                                ...
                                if (response.data.response.is_valid === true) {
                                    ...
                                    // submission not valid, so fail
                                    return deferred.reject(response);
                                }

                            } else {
                                // No response status, or status NOT successful, so fail.
                                return deferred.reject(response);
                            }
                        },
                        function (error) {
    ...
    lnOGravityFormService.$inject = ['$http', '$cacheFactory', '$q', '$log', '$lnOGravityFormConfig', 'lnOGravityFormGETFormTransformer'];
