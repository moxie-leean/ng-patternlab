...
function lnOGravityFormParser($log, isValidParameterFilter, isValidObjectAndPropertyFilter) {
   ...
   * @returns {string}
   */
  function parseField(fieldHTML, labelHTML, validationHTML, descriptionHTML, field ) {
    ...
    return '<li class="field {{class}} {{hidden}}" id={{id}}><div class="field-inner">{{label}} {{field}} {{validation}} {{description}}</div></li>'
      .replace(/{{hidden}}/gi, field.isHidden ? ' ng-hide' : '')
      .replace(/{{class}}/gi, field.cssClass )
      .replace(/{{id}}/gi, 'field_' + field.formId + '_' + field.fieldId)
