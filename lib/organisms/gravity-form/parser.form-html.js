...
function lnOGravityFormParser($log, isValidParameterFilter, isValidObjectAndPropertyFilter) {
    ...
    angular.forEach(fieldsData, function (field) {
      ...
      if (angular.isDefined(_method)) {
        ...
        if (isValidParameterFilter(_fieldHTML)) {
          ...
          _fields += parseField(_fieldHTML, _labelHTML, _validationHTML, _descriptionHTML, field.hidden, field.formId, field.id);
        }
      } else {
   ...
   * @returns {string}
   */
  function parseField(fieldHTML, labelHTML, validationHTML, descriptionHTML, isHidden, formId, fieldId) {
    return '<li class="field{{hidden}}" id={{id}}><div class="field-inner">{{label}} {{field}} {{validation}} {{description}}</div></li>'
