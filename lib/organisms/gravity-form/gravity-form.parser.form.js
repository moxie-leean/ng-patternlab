...
function lnOGravityFormParser($log, isValidParameterFilter) {
  ...
  function parseGroupInputList(data, startIndex, nameHasIndexSuffix) {
    var _list = '<ul id="input_' + data.formId + '_' + data.id + '">';
  ...
  function parseAllFields(fieldsData) {
    ...
    angular.forEach(fieldsData, function (field) {
      ...
      field.originalId = angular.copy(field.id);
      ...
      switch (field.type) {
        ...
        case 'hidden':
          ...
          field.id = 'input_' + field.formId + '_' + field.id;
          field.name = isValidParameterFilter(field.name) ? field.name : 'input_' + field.originalId;
      ...
      if (angular.isDefined(_fieldHTML)) {
        console.log(field.id,  '/ isRequired: ', field.isRequired);
        _labelHTML = (!_hidden && _hasLabel) ? parseLabel(field.id, field.label, field.isRequired) : '';
        _fields += parseField(_fieldHTML, _labelHTML, _hidden, field.formId, field.originalId);
      }
    });
