 ...
 *
 *    - Make sure pre-selected options are rendering properly
 *    - Build a regex map for common regex replaces
 */
...
function lnOGravityFormParser($log, isValidParameterFilter) {
  ...
  function parseValidationBlock(data, formName) {
    return '<div id="validator-{{formName}}-{{fieldName}}" class="validation-block" value="{{id}}" style="display:none"><p>{{message}}<p></div>'
